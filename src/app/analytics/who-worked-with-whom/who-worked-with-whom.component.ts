...
@Component({
  selector: 'app-who-worked-with-whom',
  templateUrl: './who-worked-with-whom.component.html'
})
export class WhoWorkedWithWhomComponent implements OnInit, OnDestroy {
  ...
  url = `${environment.settings.tableauWhoWorkedWithWhomReportUrl}`;
  ...
  private loadTableauDashboard() {
    this.placeholderDiv = document.getElementById('vizContainer4W');
  ...
  private setTableauFilters(filterOptions) {
    ...
    // this.options['Operating Office Name'] = filterOptions.selectedOfficeNames; //need abbrevaition in this report and not name
    // this.options['Staffing Tag'] = filterOptions.selectedStaffingTags;
    // this.options['Level Grade'] = !this.userPreferences.levelGrades ? '' : this.userPreferences.levelGrades;
  }
}
